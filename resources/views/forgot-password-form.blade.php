...
<head>
  ...
  <title>Lupa Password | Studio Fotografi Kreatif</title>
  ...
  <div class="forgot-container">
    <div class="forgot-card text-center">
      <div class="logo-section">
        ...
        <h3>Lupa Password?</h3>
        <p class="subtitle">Jangan khawatir! Masukkan email Anda dan kami akan mengirimkan link reset password</p>
      </div>
      ...
      @if (session('success'))
        <div class="alert alert-success py-3 mb-3">
          ...
          @if (session('reset_link'))
            <div class="reset-link-box mt-2">
              <strong>Link Reset Password (Development):</strong><br>
              <a href="{{ session('reset_link') }}" target="_blank" class="link-fun">{{ session('reset_link') }}</a>
            </div>
          @endif
        </div>
      ...
      <form method="POST" action="{{ route('password.email') }}" id="forgotForm">
        ...
        <button type="submit" class="btn btn-send mt-2" id="sendBtn">
          🚀 Kirim Link Reset
        </button>
      </form>
