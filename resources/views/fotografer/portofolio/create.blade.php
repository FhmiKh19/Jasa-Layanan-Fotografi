...
<div class="row">
        ...
        <div class="card">
            <div class="card-body">
                ...
                <div class="alert alert-warning">
                    ...
                    Anda belum memiliki kategori. <a href="{{ route('fotografer.portofolio.kategori.create') }}">Buat kategori</a> terlebih dahulu.
                </div>
                @else
                <form action="{{ route('fotografer.portofolio.store') }}" method="POST" enctype="multipart/form-data" id="uploadForm">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mb-3">
                            ...
                            <select name="kategori_id" id="kategoriSelect" class="form-select @error('kategori_id') is-invalid @enderror" required>
                                ...
                                @foreach($kategoris as $kategori)
                                    <option value="{{ $kategori->id_kategori }}" 
                                            {{ old('kategori_id', request('kategori_id')) == $kategori->id_kategori ? 'selected' : '' }}>
                            ...
                            <div class="alert alert-warning mb-0">
                                ...
                                Belum ada kategori. <a href="{{ route('fotografer.portofolio.kategori.create') }}" class="alert-link">Buat kategori</a> terlebih dahulu.
                            </div>
                            @endif
                            @error('kategori_id')
                    ...
                    <div class="mb-3">
                        ...
                        <div class="upload-zone" id="uploadZone" style="border: 2px dashed var(--border-color); border-radius: 10px; padding: 40px; text-align: center; cursor: pointer; transition: all 0.3s ease;">
                            ...
                            <p class="text-secondary mt-2 mb-0">Klik atau drag & drop foto ke sini</p>
                            <p class="text-secondary" style="font-size: 12px;">Format: JPG, PNG, GIF, WebP. Maksimal 5MB per foto</p>
                            <input type="file" name="gambars[]" id="gambarInput" multiple accept="image/*" 
                                   class="d-none @error('gambars') is-invalid @enderror @error('gambars.*') is-invalid @enderror" required>
                        </div>
                        @error('gambars')
                        ...
                        <!-- Preview Container -->
                        <div id="previewContainer" class="row mt-3" style="display: none;"></div>
                        
                        <!-- Selected count -->
                        <div id="selectedCount" class="text-secondary mt-2" style="font-size: 13px; display: none;">
                            <i class="bi bi-check-circle text-success me-1"></i>
                            <span></span> foto dipilih
                        </div>
                    </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="card">
            <div class="card-body">
                <h6 class="mb-3"><i class="bi bi-folder me-2"></i>Kategori Anda</h6>
                <div class="d-flex flex-wrap gap-2">
                    @foreach($kategoris as $kategori)
                    <span class="badge badge-primary">{{ $kategori->nama_kategori }}</span>
                    @endforeach
                </div>
                <a href="{{ route('fotografer.portofolio.kategori.create') }}" class="btn btn-sm btn-outline-primary w-100 mt-3">
...
function updatePreview() {
    ...
    if (selectedFiles.length > 0) {
        ...
        selectedCount.querySelector('span').textContent = selectedFiles.length;
        ...
        selectedFiles.forEach((file, index) => {
            ...
            reader.onload = (e) => {
                ...
                col.innerHTML = `
                    <div class="preview-item" style="border: 1px solid var(--border-color); border-radius: 12px; overflow: hidden; background: linear-gradient(135deg, #191c24 0%, #1a1e26 100%); min-height: 150px; display: flex; align-items: center; justify-content: center; position: relative;">
                        <img src="${e.target.result}" 
                             alt="Preview ${index + 1}" 
                             style="max-width: 100%; max-height: 150px; width: auto; height: auto; object-fit: contain; object-position: center; padding: 8px;">
                        <button type="button" class="remove-btn" onclick="removeFile(${index})" title="Hapus">
                            <i class="bi bi-x"></i>
                        </button>
                        ${index === 0 ? '<span class="position-absolute bottom-0 start-0 m-2 badge" style="background: linear-gradient(135deg, #6c5ce7 0%, #a29bfe 100%); color: #fff; font-size: 10px; font-weight: 600; padding: 4px 8px; border-radius: 6px;"><i class="bi bi-star-fill me-1"></i> Cover</span>' : ''}
                    </div>
                `;
